...
const haijsServer = require('../server').haijsServer
...
function createApp(db, routes, middlewares, auth, argv) {
  const app = haijsServer.create()

  const { foreignKeySuffix } = argv

  let router = haijsServer.router(
    db,
  ...
  const defaults = haijsServer.defaults(defaultsOpts)
  app.use(defaults)

  if (routes) {
    const rewriter = haijsServer.rewriter(routes)
    app.use(rewriter)
  }
...
module.exports = function(argv) {
  ...
  function start(cb) {
    ...
    return load(source).then(db => {
      ...
      let auth
      if (argv.auth) {
        console.log(chalk.gray('Loading', argv.auth))
      ...
      app = createApp(db, routes, middlewares, auth, argv)
          ...
          console.log(
            chalk.red(
              `Cannot bind to the port ${
                error.port
              }. Please specify another port number either through --port argument or through the hai-server.json configuration file`
            )
          )
  ...
  start()
    .then(() => {
      ...
      // Support nohup
      // https://github.com/sumn2u/hai-server/issues/221
      ...
      if (argv.watch) {
        ...
        fs.watch(watchedDir, (event, file) => {
          // https://github.com/sumn2u/hai-server/issues/420
          // file can be null
          if (file) {
